...
func CreateStdPayloads(iss string, sub string, aud string, jti string, expSeconds uint64) map[string]interface{} {
	now := time.Now()
...
//	@return string 凭证字符串
//	@return error
func BuildTokenWithGM(payloads map[string]interface{}, exp time.Time, priKey *sm2.PrivateKey) (string, error) {
	if payloads == nil {
...
//	@return map[string]string 凭证有效负载
//	@return error
func CheckTokenWithGM(token string, pubKey *sm2.PublicKey) (map[string]interface{}, error) {
